...
void verify(void (*base_fn)(), void(*fn)()) {
    ...
    ddi_2* in_backup_ddi2 = (ddi_2*) aligned_alloc(32, LEN*sizeof(ddi_2));
    ddi_2* out_backup_ddi2 = (ddi_2*) aligned_alloc(32, LEN*sizeof(ddi_2));
    ddi_2* base_out_ddi2 = (ddi_2*) aligned_alloc(32, LEN*sizeof(ddi_2));
    ...
    int* in_backup_bool = (int*) aligned_alloc(32, LEN*sizeof(int));
    int* out_backup_bool = (int*) aligned_alloc(32, LEN*sizeof(int));
    int* base_out_bool = (int*) aligned_alloc(32, LEN*sizeof(int));
    ...
    memcpy(in_backup_ddi2, in_ddi2, LEN*sizeof(ddi_2));
    memcpy(out_backup_ddi2, out_ddi2, LEN*sizeof(ddi_2));
    ...
    memcpy(in_backup_bool, in_bool, LEN*sizeof(int));
    memcpy(out_backup_bool, out_bool, LEN*sizeof(int));
    ...
    memcpy(in_ddi2, in_backup_ddi2, LEN*sizeof(ddi_2));
    memcpy(out_ddi2, out_backup_ddi2, LEN*sizeof(ddi_2));
    ...
    memcpy(in_bool, in_backup_bool, LEN*sizeof(int));
    memcpy(out_bool, out_backup_bool, LEN*sizeof(int));
    ...
    for(int j = 0; j < LEN; ++j){
        ...
        // ddi2
        for(int k = 0; k < 2; ++k){
            u_ddi base = {.v = base_out_ddi2[j].f[k]};
            u_ddi tested = {.v = out_ddi2[j].f[k]};
            fesetround(FE_UPWARD);
            double base_lo = base.lh + base.ll;
            double base_hi = base.uh + base.ul;
            double tested_lo = tested.lh + tested.ll;
